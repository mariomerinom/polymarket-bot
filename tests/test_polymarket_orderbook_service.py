...
def _cache_file(tmp_path, tokens):
    path = tmp_path / "live_orderbook.json"
    path.write_text(json.dumps({"version": 2, "tokens": tokens}))
    return path
...
def test_get_executable_book_requires_exact_side_token(tmp_path):
    ...
    service = PolymarketOrderbookService(cache_path=cache_path)
...
def test_get_executable_book_marks_old_side_stale(tmp_path):
    ...
    service = PolymarketOrderbookService(cache_path=cache_path, max_age_s=2)
