...
| 19 | Time-of-day gate: monitor hour 16 UTC | Hour 16 WR < 40% at 50+ bets → add to DEAD_HOURS_UTC | 5m | MONITORING | 8 bets, 25% WR at hour 16 UTC. Below 50-bet minimum — monitor, don't gate yet. If confirmed at 50 bets, add to DEAD_HOURS_UTC in predict.py (frozen file — requires approval). |
| 20 | Demote 15m conv=4 to conv=3 | 15m conv=4 WR < 60% at 30+ resolved bets | 15m | READY | 59.3% WR on 27 bets (below 60% threshold). Close to trigger — 3 more bets will confirm. Distinct from #7 (which was about sizing tiers). This is about conviction accuracy: conv=4 performing no better than conv=3 on 15m pipeline. Action: cap 15m conviction at 3 or demote conv=4→conv=3 in predict_15m.py. |
| 21 | Dynamic estimates replace hardcoded 0.62/0.38 | Immediate — fundamental calibration fix | All | ACTIONED | Estimates now computed from streak length + price magnitude + volatility via strength_signal(). BTC: 0.50-0.64 range (max_edge=0.14). ETH: 0.50-0.58 (max_edge=0.08). Edge gate in trade.py now functional. Revert if WR < 55% on 50 post-change bets. |
